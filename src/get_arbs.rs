...
pub async fn get_arbs() -> Result<HashMap<String,ArbPair>, reqwest::Error> {
    let btc_price_res = reqwest::get("https://api.binance.com/api/v3/avgPrice?symbol=BTCUSDT").unwrap().text().unwrap();
    let btc_price_json: Value = serde_json::from_str(&btc_price_res).unwrap();
    let btc_price: f64 = btc_price_json.get("price").unwrap().as_str().unwrap().parse().unwrap();
    // println!("{:?}", btc_price);


    let ticker_res = reqwest::get("https://api.binance.com/api/v3/ticker/24hr").unwrap().text().unwrap();
    let ticker_json: Vec<Value> = serde_json::from_str(&ticker_res).unwrap();
    ...
    let exchange_info_res = reqwest::get("https://api.binance.com/api/v3/exchangeInfo").unwrap().text().unwrap();
    let exchange_info_serde: Value = serde_json::from_str(&exchange_info_res).unwrap();
